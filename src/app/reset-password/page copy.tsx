...
export default function ResetPassword() {
  ...
  const handleRequest = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
      setMessage(
        `OTP sent to ${/^\d/.test(identifier) ? "phone" : "email"}. Please check your ${/^\d/.test(identifier) ? "phone" : "email"}.`
      );
    }, 1200);
  };
  ...
  const handleOtp = (e: React.FormEvent) => {
    ...
    // Simulate backend OTP verification (replace with real API call)
    setIsLoading(true);
    setTimeout(() => {
      setIsLoading(false);
      // Simulate success; in real app, check backend response
      setStep("password");
      setError(null);
      setMessage(null);
    }, 1200);
  ...
  const handleReset = (e: React.FormEvent) => {
    ...
    // Send OTP and new password to backend (simulate with console.log/fetch)
    setIsLoading(true);
    setTimeout(() => {
      // Simulate API call
      console.log('Sending to backend:', { identifier, otp, newPassword });
      setIsLoading(false);
